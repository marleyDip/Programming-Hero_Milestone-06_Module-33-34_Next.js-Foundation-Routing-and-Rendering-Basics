        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((topic, i) => {
              const isDone = completed.has(topic.title);
              return (
                <div
                  key={topic.title}
                  onClick={() => toggleComplete(topic.title)}
                  style={{ animationDelay: `${i * 40}ms` }}
                  className="group relative animate-[fadeIn_0.4s_ease-out_forwards] cursor-pointer overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5 opacity-0 backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-2xl"
                >
