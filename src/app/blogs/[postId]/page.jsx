...
// interface BlogPost { id: number; title: string; description: string; author: string; category: string; date: string; image: string; }
...
export async function generateMetadata({ params }) {
  ...
  const post = blogsData.find((blog) => blog.id === Number(postId));
  ...
  /* A shorter description works better as a search/social description. The complete description is still displayed on the page. */
...
const PostDetailPage = async ({ params }) => {
  ...
  const post = blogsData.find((post) => post.id === parseInt(postId, 10));

  /* If the post doesn't exist, Next.js will render app/blogs/not-found.tsx */
  // if (!post) notFound();
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="rounded-3xl border border-dashed border-slate-700 bg-slate-900/60 px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-slate-400">
              ...
              <span className="font-semibold text-slate-300"> {postId} </span> .
  ...
  /* Structured Data - Helps search engine understand that this page represents a blog article */
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-12 text-white sm:py-16">
      <div className="mx-auto max-w-5xl">
        ...
        <Link
          ...
          className="group mb-8 inline-flex items-center gap-2 text-sm font-medium text-slate-400 transition hover:text-pink-400"
        >
          <span className="transition-transform group-hover:-translate-x-1 group-hover:scale-125 group-hover:-rotate-360 duration-500">
            ←
          </span>
          Back to Blogs
        </Link>
        ...
        {post ? (
          <article
            className={`${styles.blogCard} overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 shadow-2xl`}
          >
            {/* Cover Image */}
            ...
            <div className="p-6 sm:p-8 lg:p-10">
              ...
              <div className="mt-7 max-w-4xl">
                <p className="text-base leading-8 text-slate-400 text-left md:text-justify sm:text-lg">
                  {post.description}
                </p>

                <p className="mt-5 text-base leading-8 text-slate-400 text-left md:text-justify">
              ...
              <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-slate-800 pt-6">
                ...
                >
                  View All Posts
                  <span className="transition-transform group-hover:translate-x-1 group-hover:scale-125 duration-300">
                    →
                  </span>
                </Link>
              </div>
